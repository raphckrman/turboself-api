...
export const searchEstablishments = async (query: string, code: string, limit: number, minimalist: boolean = false, token?: string): Promise<Establishment[]> => {
  const rawEstablishmentSearch = await manager.get<rawEstablishmentSearchResult[]>(ESTABLISHMENT_SEARCH(query, code, limit), {
...
export const getEstablishmentBy2P5 = async (token: string, code2p5: string): Promise<Establishment> => {
  const rawEstablishmentGet = await manager.get<rawEstablishmentResult[]>(ESTABLISHMENT_BY_CODE(code2p5), {
    Authorization: `Bearer ${token}`
  });
